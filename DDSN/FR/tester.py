...
class Tester(Trainer):
    ...
    def test(self):
        ...
        tic = time.time()
        with torch.no_grad():
            for i, (img, gt, img_name) in enumerate(tbar):  # 添加 img_name
                ...
                if self.show:
                    ...
                    # 使用原始图片名字保存
                    base_name = os.path.splitext(img_name[0])[0]
                    cv2.imwrite(
                        f"save_picture/img_{base_name}.png", np.uint8(img.cpu().numpy() * 255))
                    cv2.imwrite(
                        f"save_picture/gt_{base_name}.png", np.uint8(gt.cpu().numpy() * 255))
                    cv2.imwrite(
                        f"save_picture/pre_{base_name}.png", np.uint8(predict * 255))
                    cv2.imwrite(
                        f"save_picture/pre_b_{base_name}.png", np.uint8(predict_b * 255))
