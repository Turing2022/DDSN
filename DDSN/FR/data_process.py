...
def data_process(data_path, name, patch_size, stride, mode):
    ...
    for i, file in enumerate(file_list):
        if name == "DRIVE":
            ...
            if mode == "training" and int(file[:-4]) <= 20:
                ...
                img = cv2.imread(os.path.join(img_path, file), 0)
                gt = cv2.imread(os.path.join(gt_path, "angio" + file[:-4] + "ok.png"), 0)
                gt = np.where(gt >= 100, 255, 0).astype(np.uint8)
                img = cv2.resize(img, (512, 512), interpolation=cv2.INTER_LINEAR)
                img_list.append(ToTensor()(img))
                gt_list.append(ToTensor()(gt))
    ...
    if mode == "training":
        ...
        save_each_image_with_name(img_list, save_path, "img", name, file_names)
        save_each_image_with_name(gt_list, save_path, "gt", name, file_names)
...
def save_patch(imgs_list, path, type, name):
    for i, sub in enumerate(imgs_list):
        with open(os.path.join(path, f'{type}_{i}.pkl'), 'wb') as file:
            pickle.dump(np.array(sub), file)
            print(f'save {name} {type} : {type}_{i}.pkl')


def save_each_image_with_name(imgs_list, path, type, name, file_names):
    for i, sub in enumerate(imgs_list):
        # 使用原始文件名保存
        file_name = os.path.splitext(file_names[i])[0]  # 去掉扩展名
        with open(os.path.join(path, f'{type}_{file_name}.pkl'), 'wb') as file:
            pickle.dump(np.array(sub), file)
            print(f'save {name} {type} : {type}_{file_name}.pkl')
